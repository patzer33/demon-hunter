...
Game.onReturnToMenu = returnToMainMenu;
...
window.addEventListener('keydown', (e) => {
  ...
  // ESC: se uma sub-tela da pausa estiver aberta, fecha ela primeiro; senão, alterna a pausa.
  if (key === 'escape' && !Game.gameOver) {
    if (Game.paused && Game.pauseSubScreen) {
      Game.pauseSubScreen = null;
    } else {
...
canvas.addEventListener('mousemove', (e) => {
  ...
  } else if (currentScreen === 'playing' && Game.paused) {
    Game.handlePauseMouseMove(mouseWorld.x, mouseWorld.y);
  } else if (currentScreen === 'playing' && (Game.gameOver || Game.victory)) {
    Game.handleEndScreenMouseMove(mouseWorld.x, mouseWorld.y);
  }
});
...
canvas.addEventListener('mousedown', (e) => {
  ...
  if (currentScreen === 'playing' && Game.paused) {
    Game.handlePauseClick(mouseWorld.x, mouseWorld.y);
    return;
  }

  if (currentScreen === 'playing' && (Game.gameOver || Game.victory)) {
    Game.handleEndScreenClick(mouseWorld.x, mouseWorld.y);
    return;
  }

  if (currentScreen === 'playing' && !Game.gameOver) {
